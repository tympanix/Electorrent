    ...
    async getTorrentDetails(hash: string): Promise<BittorrentTorrentDetailsData> {
        ...
        return {
            info: {
                ...
                availability: Array.isArray(torrent.availability) && torrent.availability.length > 0
                    ? torrent.availability.filter((piece: unknown) => Number(piece) !== 0).length / torrent.availability.length
                    : null,
                activityDate: torrent.activityDate ?? null,
                editDate: torrent.editDate ?? null,
                startDate: torrent.startDate ?? null,
                bandwidthPriority: torrent.bandwidthPriority ?? null,
                corruptEver: torrent.corruptEver ?? null,
                desiredAvailable: torrent.desiredAvailable ?? null,
                etaIdle: torrent.etaIdle ?? null,
                fileCount: torrent['file-count'] ?? null,
                group: torrent.group ?? null,
                haveUnchecked: torrent.haveUnchecked ?? null,
                haveValid: torrent.haveValid ?? null,
                honorsSessionLimits: torrent.honorsSessionLimits ?? null,
                isFinished: torrent.isFinished ?? null,
                isStalled: torrent.isStalled ?? null,
                leftUntilDone: torrent.leftUntilDone ?? null,
                magnetLink: torrent.magnetLink ?? null,
                metadataPercentComplete: torrent.metadataPercentComplete ?? null,
                percentComplete: torrent.percentComplete ?? null,
                primaryMimeType: torrent['primary-mime-type'] ?? null,
                secondsDownloading: torrent.secondsDownloading ?? null,
                secondsSeeding: torrent.secondsSeeding ?? null,
                seedIdleLimit: torrent.seedIdleLimit ?? null,
                seedIdleMode: torrent.seedIdleMode ?? null,
                sizeWhenDone: torrent.sizeWhenDone ?? null,
                torrentFile: torrent.torrentFile ?? null,
                webseeds: Array.isArray(torrent.webseeds) ? torrent.webseeds.length : null,
                webseedsSendingToUs: torrent.webseedsSendingToUs ?? null,
                peersGettingFromUs: torrent.peersGettingFromUs ?? null,
                peersSendingToUs: torrent.peersSendingToUs ?? null,
                peersFromCache: torrent.peersFrom?.fromCache ?? null,
                peersFromDht: torrent.peersFrom?.fromDht ?? null,
                peersFromIncoming: torrent.peersFrom?.fromIncoming ?? null,
                peersFromLpd: torrent.peersFrom?.fromLpd ?? null,
                peersFromLtep: torrent.peersFrom?.fromLtep ?? null,
                peersFromPex: torrent.peersFrom?.fromPex ?? null,
                peersFromTracker: torrent.peersFrom?.fromTracker ?? null,
            },
        }
    }
